...
from config import (
    ...
    OU_MIN_GAMES, OU_MODEL_PATH, TOTAL_MODEL_PATH, WIN_MODEL_PATH,
)
...
def main() -> None:
    ...
    parser.add_argument("--tune", action="store_true",
                        help="Run Optuna hyperparameter search before training (monthly)")
    ...
    from model.train import load_data, train_and_evaluate, tune_hyperparams
    ...
    win_params = total_params = None
    if args.tune:
        logger.info("Running Optuna hyperparameter tuning (%d trials)...", args.tune_trials)
        win_params, total_params = tune_hyperparams(
            X, y_win, y_total, sample_weight,
            n_trials=args.tune_trials, cv_folds=args.cv_folds,
        )

    clf, reg, calibrator, cv_metrics = train_and_evaluate(
        X, y_win, y_total,
    ...
    logger.info("Win + run-total models and calibrator saved")
    ...
    if n_ou >= OU_MIN_GAMES:
        ...
        if feat_path.exists() and res_path.exists():
            ...
            # Append live 2026 games so ou_line game_ids (2026) match
            ...
            if not live_df.empty:
                ...
                logger.info("OU feature set: %d games total (incl. %d live)",
                            len(features_df), len(live_df))
    ...
    notify_retrain({
        ...
        "win_brier_cal":   cv_metrics.get("brier_cal"),
